...
import {
  ...
  TrendingUp, TrendingDown, Upload, Target, Footprints, Gamepad2, Save, Clock,
} from "lucide-react";
...
const OfficialProfile = () => {
  ...
  const {
    ...
    profilePics, attendance, currentWeekStart,
  ...
  const showContributions = !isFabian; // Fabian exempt

  const playerMembers = members.filter((m) => m.role === "player" || m.role === "captain");

  // Dynamic scorer dropdowns based on our score
  ...
  const handleMediaUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    toast({ title: "Uploading...", description: `Uploading ${files.length} photo(s)...` });
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="max-w-5xl mx-auto px-4 py-8 space-y-6">
        ...
        <div className="grid md:grid-cols-2 gap-6">
          {/* Game Scores with Scorer Selection */}
          {canManageScores && (
        ...
        {/* Official's own contribution section (except Fabian) */}
        {showContributions && (
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.28 }}>
            <Card className="bg-card border-border card-glow">
              <CardHeader>
                <CardTitle className="font-heading text-lg text-foreground">My Monthly Contributions</CardTitle>
              </CardHeader>
              <CardContent className="space-y-3">
        ...
        {/* Fadhir's Direct Contribution Checkbox Grid */}
        {isFadhir && (
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.32 }}>
            <Card className="bg-card border-border card-glow">
              <CardHeader>
                <CardTitle className="font-heading text-lg text-foreground flex items-center gap-2">
                  <CheckCircle className="w-5 h-5 text-primary" /> Contribution Management
                </CardTitle>
                              ...
                              <td key={month.key} className="py-2 text-center">
                                <Checkbox checked={status === "paid"}
                                  onCheckedChange={(checked) => {
                                    updateContributionDirect(m.id, month.key, checked ? "paid" : "unpaid");
                                  }} />
                              </td>
                            );
                          })}
        ...
        {/* Weekly Attendance Tracker (Manager & Coach) */}
        {(isManager || isCoach) && (
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.38 }}>
            <Card className="bg-card border-border card-glow">
              <CardHeader>
                <CardTitle className="font-heading text-lg text-foreground flex items-center gap-2">
                  📋 Weekly Attendance — {currentWeekStart}
                </CardTitle>
                              ...
                              return (
                                <td key={day} className="py-2 text-center">
                                  <Checkbox checked={status === "present"}
                                    ...
                                    onCheckedChange={(checked) => {
                                      updateAttendance(m.id, day, checked ? "present" : "absent");
                                    }} />
                                  {status === "excused" && <span className="text-xs text-primary block">E</span>}
                                  {status === "no_activity" && <span className="text-xs text-muted-foreground block">—</span>}
                                </td>
                              );
                            })}
